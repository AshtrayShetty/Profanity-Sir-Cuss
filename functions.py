...
def album_song_list(artists_links, album_song_names, song_links, writer):
    if not path.getsize('artists_links.txt')==0:
        try:
            time.sleep(random.uniform(1,6))
            albums_list=requests.get(f'https://wwww.azlyrics.com/{artists_links.readline()}').text
            album_song_names
        except:
        ...
        del_first_link()
        return album_song_list(artists_links, album_song_names, song_links, writer)
